...
st.set_page_config(page_title="CCCD Extraction", page_icon="🪪", layout="wide")

# --- Khởi tạo Model (Dùng cache để không load lại khi refresh) ---
@st.cache_resource
def load_models():
    ...
    qr_det = QRDetector()
    return yolo, vietocr, qr_det

yolo_model, vietocr_predictor, qr_detector = load_models()
...
def postprocess_text(field, text):
    ...
    if field == 'id_number':
        ...
        for c_no, c_org in COUNTRIES_NO_ACCENT.items():
            if c_no in text_no_accent: return c_org
        text = re.sub(r'\d+', '', text)
        return text.strip(" ,.-").strip()
        
    elif field == 'gender':
        text_lower = text.lower()
        if 'nam' in text_lower: return 'Nam'
        elif 'nữ' in text_lower or 'nu' in text_lower: return 'Nữ'
        text = re.sub(r'\d+', '', text)
        return text.strip(" ,.-").strip()
        
    elif field in ['dob', 'expiry']:
        if field == 'expiry' and any(k in text.lower() for k in ['vô', 'không', 'thời', 'hạn']):
...
def fix_orientation(card_img):
    ...
    qr_center_x, qr_center_y = (x1 + x2) / 2, (y1 + y2) / 2
...
def run_pipeline(img_rgb):
    ...
    if not field_boxes:
        return annotated, result_json, False
    ...
    for field, boxes_list in field_boxes.items():
        ...
        for xyxy in boxes_sorted:
            ...
            if text: line_texts.append(text)
    ...
    return annotated, result_json, True

# --- Streamlit UI ---
st.title("🪪 Trích xuất thông tin CCCD")
st.markdown("Hỗ trợ tự động căn chỉnh ảnh, nhận diện các trường thông tin và làm sạch kết quả tiếng Việt.")

col1, col2 = st.columns([1, 1])

with col1:
    uploaded_file = st.file_uploader("Upload ảnh thẻ CCCD", type=["jpg", "jpeg", "png"])
...
if uploaded_file is not None:
    ...
    with st.spinner('Đang phân tích và xử lý ảnh...'):
        annotated_img, results_data, success = run_pipeline(img_rgb)
    
    with col1:
        st.image(annotated_img, caption="Ảnh đã xử lý (Cắt & Xoay chuẩn)", use_container_width=True)
        if not success:
            st.error("⚠️ Không nhận diện được thẻ CCCD trong ảnh. Vui lòng kiểm tra lại!")
        else:
            key_fields = ['id_number', 'name', 'dob']
            if sum(1 for f in key_fields if results_data.get(f)) < 2:
                st.warning("⚠️ Nhận diện được thẻ nhưng thông tin bị mờ/khuất, kết quả có thể không đầy đủ.")

    with col2:
        st.subheader("Kết quả trích xuất")
        for key, label in FIELD_LABELS.items():
            st.text_input(label, value=results_data.get(key, ""), disabled=False)
